...
(function () {
    ...
    _app.studio = {
        ...
        inspect: function (x, y) {
            ...
            var elem = $(document.elementFromPoint(x, y)),
                ...
                target;
            if (elem.is('.app-wrapper') && elem.closest('.app-page-modal').length) {
                // The header of the modal form is hidden under the wrapper. Bring it forward temprorarily for inspection
                var modalPageHeader = elem.closest('.ui-page').find('.app-page-header:first');
                if (modalPageHeader.length) {
                    modalPageHeader.css('z-index', parseFloat(elem.css('z-index')) + 1).find('i').css('z-index', -1);
                    elem = $(document.elementFromPoint(x, y));
                    modalPageHeader.css('z-index', '').find('i').css('z-index', '');
                }
            }
            if (inspectorConfig) {
                _frame.css('left', '');
                inspectorConfig.every(function (t) {
                    try {
                        ...
                        if (el.length && (!t.skip || !elem.closest(t.skip).length) && (!t.keep || el.closest(t.keep).length)) {
                            elem = el;
                            type = t;
                            navigate = t.navigate;
                            // create a selector
                ...
                if (typeof navigate == 'function')
                    navigate = navigate.call(selector, target);
    ...
    $(document).on('vclick', '.studio #studio .ui-content', function (e) {
        ...
        if (isStudio) {
            if (!location.hash) {
                ...
                e.context.push(
                    ...
                    {
                        studio: true,
                        text: 'Edit Project Code',
                        icon: 'material-icon-edit-note',
                    ...
                    },
                    {
                        ...
                        icon: 'material-icon-folder-code',
                        context: { action: 'open' },
                        callback: studioActionHandler
                    },
                    {
                ...
                e.context.push(
                    {
                        studio: true,
                        text: 'Reset Session',
                        context: { action: 'startover' },
                        icon: 'material-icon-history',
                        callback: studioActionHandler
                    },
                );
        ...
        return false;
    }).on('studioactionstartover.app', e => {
        if (!isStudio) {
            appStudioStart({ event: { type: 'studioactionstartover.app' } });
        }
        else {
            _app.touch.show({
                text: 'Reset Session',
                ...
                topics: [
                    {
                        ...
                        questions: [
                            {
                                text: false,
                                value: 'This will clear all your current settings, filters, and selections.<br/>The app design will remain the same.<br/><br/>Are you sure you want to continue?',
                                readOnly: true,
                                htmlEncode: false,
                                options: {
                                    textStyle: 'primary'
                ...
                .then(result => {
                    _touch.notify('Starting over now...');
                    setTimeout(() => {
                        try {
                            clearAppState(localStorage);
                            clearAppState(sessionStorage);
                        }
                        catch (ex) {
            ...
            if (isStudio) {
                ...
                switch (treeView.eventName) {
                    ...
                    case 'get':
                        ...
                        break;
                    case 'select':
                        var nodeData = treeView.nodeData;
                        if (nodeData)
                            if (_selectionMarker.data('inspected'))
                                _selectionMarker.removeData('inspected');
                            else
                                postMessageToApp({ method: 'nodeChanged', nodeData: nodeData })
                        break;
                }
            }
        })
        .on('propgrid.app', e => {
            if (isStudio) {
                switch (e.propGrid.eventName) {
                    case 'edit':
                        e.propGrid.result = executePropGridEdit(e.propGrid, e.treeView);
                        break;
                    case 'changed':
                        alterUI(e.propGrid.target, e.propGrid.propMap);
                        break;
                    case 'help':
                    ...
                    case 'fetch':
                        e.propGrid.result = Promise.all([
                            ...
                            fetchObject(e.treeView.elem.find('.app-selected').length > 1 ? null : e.treeView.nodeType)
                        ]).catch(studioError);
                        break;
                    case 'hypermedia':
                        e.propGrid.result = studioRestful(e.propGrid.eventArgs)
                            .then(result => {
                                if (e.propGrid.trigger !== 'none') {
                                    var trigger = e.propGrid.trigger || calculateStudioTrigger();
                                    if (trigger != 'none')
    ...
    function alterUI(targetObj, propMap) {
        var uiAlterationScripts = [];
        var trigger;
        for (var key in propMap) {
            var propDef = propMap[key];
            trigger = calculateStudioTrigger(trigger, propDef, targetObj)
    ...
    function calculateStudioTrigger(trigger, newTrigger, targetObj) {
        var trigger;
        var propDef;
        if (typeof newTrigger == 'object') {
            ...
            if (propDef.alterApp)
                ...
                if (propDef.trigger != null) {
                    newTrigger = _app.eval(propDef.trigger, targetObj || {});
                }
                else
                    newTrigger = 'reload';
            }
    ...
    function studioError(ex) {
        _touch.notify(typeof ex == 'string' ? ex : studioErrorMessage(ex));
    }
    ...
    window.addEventListener('message', function (e) {
        ...
        if (method === 'inspect') {
            ...
            selectionMarker('show').data('inspected', true).css({ left: bounds.left, top: bounds.top, width: bounds.width, height: bounds.height });
            if (navigatePath) {
                ...
                showInspectionResultHint(e.data.selector);
        ...
        else if (method == 'nodeChanged') {
            if (!isStudio)
                selectInspectedNode(e.data.nodeData);
        }
        else if (method === 'marker') {
            if (isStudio) {
                var markerBounds = e.data.bounds;
                var marker = selectionMarker();
                if (marker.data('inspected'))
                    marker.removeData('inspected'); // do not change the marker position
                else if (markerBounds)
                    selectionMarker('show').css({ left: markerBounds.left, top: markerBounds.top, width: markerBounds.width, height: markerBounds.height });
                else
                    marker.hide();
            }
        }
    ...
    function selectInspectedNode(nodeData) {
        var selfHref = nodeData?._links?.self?.href;
        var target;
        var targetElem;
        if (selfHref) {
            // data field in a category
            target = selfHref.match(/\/controllers\/(?<controller>.+?)\/views\/(?<view>.+?)\/categories\/(?<category>.+?)\/data-fields\/(?<fieldName>.+?)(\/|$)/);
            if (!target)
                target = selfHref.match(/\/controllers\/(?<controller>.+?)\/views\/(?<view>.+?)\/data-fields\/(?<fieldName>.+?)(\/|$)/);
            // data category
            if (!target)
                target = selfHref.match(/\/controllers\/(?<controller>.+?)\/views\/(?<view>.+?)\/categories\/(?<category>.+?)(\/|$)/);
            // view
            if (!target)
                target = selfHref.match(/\/controllers\/(?<controller>.+?)\/views\/(?<view>.+?)(\/|$)/);
            // field
            if (!target)
                target = selfHref.match(/\/controllers\/(?<controller>.+?)\/fields\/(?<field>.+?)(\/|$)/);
            // action
            if (!target)
                target = selfHref.match(/\/controllers\/(?<controller>.+?)\/action-groups\/(?<actionGroup>.+?)\/actions\/(?<action>.+?)(\/|$)/);
            // actionGroup
            if (!target)
                target = selfHref.match(/\/controllers\/(?<controller>.+?)\/action-groups\/(?<actionGroup>.+?)(\/|$)/);
        }
        if (target) {
            ...
            if (target.controller) {
                ...
                if (dataView) {
                    if (dataView._controller == target.controller) {
                        if (dataView._viewId == target.view) {
                            if (target.fieldName)
                                targetElem = findInspectedField(target.fieldName);
                            else if (target.category)
                                targetElem = _touch.activePage(`[data-layout="form"] [data-category="${target.category}"]`);
                            else if (target.view)
                                targetElem = _touch.activePage();
                        }
                        else if (target.field)
                            targetElem = findInspectedField(target.field);
                        else if (target.action)
                            targetElem = _touch.activePage(`[data-action-path="${target.actionGroup}/${target.action}"]`);
                        else if (target.actionGroup) {
                            targetElem = _touch.activePage(`[data-action-path="${target.actionGroup}"]`);
                            if (!targetElem.length) {
                                if (dataView.get_viewType() == 'Form') {
                                    dataView._actionGroups.forEach(ag => {
                                        if (ag.Id == target.actionGroup) {
                                            if (ag.Scope === 'Form')
                                                targetElem = _touch.activePage(`.app-bar-buttons`);
                                        }
                                    });
                                    if (!targetElem.length)
                                        targetElem = _touch.activePage(`[data-field="${target.actionGroup}"][data-control="field"]`);
                                }
                            }
                        }
                    }
        ...
        if (targetElem?.length && targetElem.is(':visible')) {
            var scrollableBounds = _app.clientRect(_touch.scrollable());
            var bounds = _app.clientRect(targetElem);
            var scrollableTop = scrollableBounds.top + 20;
            var scrollableBottom = scrollableBounds.bottom - 20;
            if (bounds.top >= scrollableBottom && bounds.bottom >= scrollableBottom || bounds.top <= scrollableTop && bounds.bottom <= scrollableTop) {
                targetElem[0].scrollIntoView(({ block: 'nearest', behavior: 'instant' }));
                bounds = _app.clientRect(targetElem);
            }
            setTimeout(() => $('.app-bar-heading').hide(), 200);
            postMessageToStudio({ method: 'marker', bounds: bounds });
        }
        else
            postMessageToStudio({ method: 'marker', bounds: false });
    }

    function findInspectedField(fieldName) {
        var targetElem = null;
        var dataView = _touch.dataView();
        var field = dataView.findField(fieldName);
        if (field) {
            if (dataView?._view?.Type == 'Form') {
                field = dataView._allFields[field.OriginalIndex];
                targetElem = _touch.activePage(`[data-layout="form"] [data-field="${field.Name}"][data-control="${field.Type == 'DataView' ? 'dataview' : 'field'}"]`);
            }
            else {
                field = dataView._allFields[field.AliasIndex];
                targetElem = _touch.activePage(`.dv-item .app-selected .app-field-${field.Name}:visible`).first();
                if (!targetElem.length)
                    targetElem = _touch.activePage(`.dv-item .app-field-${field.Name}:visible`).first();
            }
        }
        return targetElem;
    }
    ...
    function showInspectionResultHint(selector) {
        var hint = [];
        ...
        function add(value, type) {
            if (value != null) {
                ...
                if (hint.length)
                    hint.push(' / ');
                hint.push(_app.htmlEncode(value));
            }
        }
        ...
        _touch.notify(_app.prettyText(selector.type, true) + ': ' + hint.join(''));
    ...
    function ensureStudioControls(fadeIn) {
        ...
        // develop
        _touch.icon('material-icon-edit-note',
            $div('studio-btn')
                .attr('data-studio-action', 'develop')
                .attr('title', 'Edit Project Code')
                .appendTo(toolbarTop));

        // files
        _touch.icon('material-icon-folder-code',
            $div('studio-btn')
        ...
        // start-over   
        _touch.icon('material-icon-history',
            $div('studio-btn')
                .attr('data-studio-action', 'startover')
                .attr('title', 'Reset Session')
                .appendTo(toolbarTop));
        ...
        // models
        _touch.icon('material-icon-table-view',
            $div('studio-btn')
            ...
            if (restfulOptions) {
                ...
                studioRestful(options)
                    .then(resource => {
                        ...
                        var response = _app.studio.utils.resourceToViewPage(resource, options);
    ...
    function clearAppState(storage) {
        var appPrefix = __settings.appName.replace(/\W/g, '_') + '_';
        var keysToDelete = []
        for (var key in storage)
            if (key.indexOf(appPrefix) === 0 && !key.match(/(\.|_)studio(\.|_)/))
    ...
    function selectionMarker(method) {
        if (!_selectionMarker)
            _selectionMarker = $span('studio-selector').appendTo('#studio').hide();
        if (method == 'show') {
            _selectionMarker.hide().addClass('app-animated');
            setTimeout(() => _selectionMarker.show().removeClass('app-animated'));
        }
