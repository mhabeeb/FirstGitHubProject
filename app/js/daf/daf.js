...
(function () {
    ...
    _app.prototype = {
        ...
        updated: function () {
            ...
            if (this.useCase('$app'))
                return;
            var startCommandName = this.get_startCommandName();
            if (startCommandName) {
                this.set_searchOnStart(false);
                this.set_lastCommandName(startCommandName);
                this.set_lastCommandArgument(this.get_startCommandArgument());
                var startCommandArgument = this.get_startCommandArgument();
                if (startCommandName.match(/New|Edit|Select/) && startCommandArgument)
                    this.set_viewId(startCommandArgument);
                this.set_startCommandName(null);
        ...
        goToView: function (viewId) {
            var that = this;
            if (!viewId && that._doneCallback) {
                that._doneCallback(that);
                return;
            }
            if (!isNullOrEmpty(that._replaceTriggerViewId) && that._replaceTriggerViewId == viewId) {
                if (that._skipTriggeredView) that._skipTriggeredView = false;
                location.replace(location.href)
                return;
            }
            var ditto = that._ditto && that._ditto.slice(0);
            that._clearCache(true);
            that._ditto = ditto;
            var lastFilter = that.get_filter();
            var lastGroup = that.get_view().Group;
            if (viewId === 'form')
                for (var i = 0; i < that.get_views().length; i++)
                    if (that.get_views()[i].Type === 'Form') {
                        viewId = that.get_views()[i].Id;
                        break;
                    }
            that._disposePendingUploads();
            that._detachBehaviors();
            if (!that.get_isForm() /*this.get_view().Type != 'Form'*/) {
                that._lastViewId = that.get_viewId();
                that._selectedRowIndex = 0;
            }
            var oldViewId = that.get_view().Id,
                viewChanged = oldViewId != viewId,
                wasForm,
                originalExtension = that.extension();
            if (viewChanged) {
                that._focusedFieldName = null;
                wasForm = that.get_isForm();
                if (!that.get_isGrid())
                    that.writeContext('vitals', null);
            }
            that.set_viewId(viewId);
            var v = that.get_view();
            if (v.Type !== 'Form') {
                that._lastViewId = viewId;
                that._restorePosition();
                if (viewChanged && wasForm && _touch)
                    originalExtension._dispose(true);
            }
            that.set_pageIndex(-1);
            var sameFilterGroup = !isNullOrEmpty(lastGroup) && that.get_view().Group === lastGroup
            var viewFilter = v.Type === 'Form' ? that.get_selectedKeyFilter() : (sameFilterGroup || !viewChanged ? lastFilter : []);
            if (this._requestedFilter && !sameFilterGroup) {
                viewFilter = that._requestedFilter;
                that._requestedFilter = null;
            }
            var viewSortExpression = viewChanged ? null : that.get_sortExpression();
            if (that._requestedSortExpression) {
                viewSortExpression = that._requestedSortExpression;
                that._requestedSortExpression = null;
            }
            that.set_filter(viewFilter);
            that.set_sortExpression(viewSortExpression);
            that._loadPage();
            that._raiseSelectedDelayed = true;
            if (viewChanged) {
                that._scrollIntoView = true;
                that._focusedCell = null;
            }
        },
    ...
    function hashCode(str) {
        let hash = 0; // Use regular number for compatibility
        ...
        //return  encodeBase64IE9(hash); 
        return hash.toString().replace(/\-/g, '_');
    }

    //function encodeBase64IE9(number) {
    ...
    //    while (i < 32) {
    //        chr1 = (number >> (24 - i * 8)) & 255;
    //        chr2 = (number >> (16 - i * 8)) & 255;
    //        chr3 = (number >> (8 - i * 8)) & 255;
    //        enc1 = chr1 >> 2;
                ...
                if (!that._rules) {
                    ...
                    for (i = 0; i < expressions.length; i++) {
                        ...
                        if (exp.Type === Web.DynamicExpressionType.ClientScript && exp.Scope === Web.DynamicExpressionScope.Rule) {
                            ...
                            if (m) {
                                var ruleHash = hashCode(m[6]),
                                    ruleFuncName = String.format('{0}_Rule_{1}_{2}', controllerName, m[1] ? m[1] : ruleIndex, ruleHash),
                                ...
                                if (!ruleFunc)
                                    try {
                                        ruleFunc = _businessRules._compiled[ruleHash];
                                        if (!ruleFunc) {
                                            var ruleText = that._parseRule(ruleName, m[6]);
                                            ruleFunc = _businessRules._compiled[ruleHash] = eval(ruleText);
                                        }
    ...
    _app.clipboard = function (value) {
        if (arguments.length)
            _app._clipboard = value;
