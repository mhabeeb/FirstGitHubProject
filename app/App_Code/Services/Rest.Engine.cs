...
namespace MyCompany.Services.Rest
{
    ...
    public partial class RESTfulResourceBase : RESTfulResourceConfiguration
    {
        ...
        public string SelfUrl
        {
            get
            {
                var url = new List<string>(_resourceLocation);
                if (!string.IsNullOrEmpty(ActionPathName))
                    url.RemoveAt((url.Count - 1));
                return ("/v2/" + string.Join("/", url));
            }
        }
        ...
        public List<RESTfulNavigationData> Trail
        {
            get
            {
        ...
        public FieldValue[] this[string identifier]
        {
            get
            {
                var result = LookupNavigationIdentifier(identifier);
                return result;
            }
        }
                        ...
                        else
                        {
                            if (_id == null)
                            {
                                ...
                                if (SegmentIsIdentifier(segment) || !GetView("auto", segment, out _view))
                                {
                                    ...
                                    {
                                        if ((HttpMethod == "POST") && ((FindAction(segment) != null) && !AllowActionInPath(segment)))
        ...
        public virtual ViewPage Execute(PageRequest request, JObject payload, JObject result)
        {
            ...
            if (RequiresSchemaOnly || WasMoved)
                return null;
                    ...
                    if (_fieldMap.TryGetValue(propName, out fieldNav))
                    {
                        ...
                        if (fieldNav.GetAttribute("isPrimaryKey", string.Empty) != "true")
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    var v = PropertyToValue(p, fieldNav, fieldType, allowNulls, hasDefault);
                                    var fvo = new FieldValue(fieldName);
                                    if (isReadOnly)
                                    {
                                        fvo.OldValue = v;
                            ...
                            foreach (var pkField in PK)
                                if (fieldName == pkField.Name)
                                {
                                    ...
                                    if ((HttpMethod == "POST") && ((commandName == "Insert") && !isReadOnly))
                                    {
                                        if (p.Value.Type == JTokenType.Null)
                                            RESTfulResource.ThrowError(400, true, "invalid_argument", "The value of the key field '{0}' cannot be null.", p.Name);
                                        pkField.NewValue = PropertyToValue(p, fieldNav, fieldType, allowNulls, hasDefault);
                                        pkField.Modified = true;
                                    }
                                    break;
                                }
        ...
        protected object PropertyToValue(JProperty p, XPathNavigator fieldNav, string fieldType, bool allowNulls, bool hasDefault)
        {
            object v = null;
            if (p.Value.Type != JTokenType.Null)
                try
            ...
            if (IsCollection && (Action != null))
            {
                ...
                if ((collection != null) && collection.Type != JTokenType.Null)
                {
                    ...
                    foreach (var theItem in collection)
                    {
                        ...
                        if (theItem is JObject)
                            foreach (var p in ((JObject)(theItem)).Properties())
                            {
                                XPathNavigator fieldNav = null;
                                if (_fieldMap.TryGetValue(p.Name, out fieldNav))
                                {
                                    ...
                                    {
                                        var fieldName = fieldNav.GetAttribute("name", string.Empty);
                                        var keyValue = GetPropertyValue(((JObject)(theItem)), p.Name, fieldNav.GetAttribute("type", string.Empty));
        ...
        {
            if (!string.IsNullOrEmpty(action) && Regex.IsMatch(action, "^a\\d+$"))
                return null;
        ...
        protected virtual bool AllowActionInPath(string action)
        {
            return false;
        }
    }
